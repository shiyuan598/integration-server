...
def search():
    try:
        ...
        data = generateEntries(["id", "name", "platform", "job_name", "job_name_p", "job_name_test",
            "lidar_path", "camera_path", "map_path", "plan_map_path", "lidar_point_path", "webviz_path", "testset_path", "mcu_path", "driver_path",
...
def search_all():
    try:
        ...
        query = session.query(Project.id, Project.name, Project.platform, Project.job_name, 
        Project.job_name_p, Project.job_name_test, Project.lidar_path, Project.camera_path, Project.map_path, 
        Project.plan_map_path, Project.lidar_point_path, Project.webviz_path, Project.testset_path, Project.mcu_path, Project.driver_path, Project.sdc_path,
        ...
        session.close()
        data = generateEntries(["id", "name", "platform", "job_name", "job_name_p", "job_name_test",
            "lidar_path", "camera_path", "map_path", "plan_map_path", "lidar_point_path", "webviz_path", "testset_path", "mcu_path",
...
def create():
    try:
        ...
        testset_path = request.json.get("testset_path")
        ...
        data = Project(name=name, platform=platform,job_name=job_name, job_name_p=job_name_p,
            ...
            testset_path=testset_path, mcu_path=mcu_path, driver_path=driver_path, sdc_path=sdc_path, owner=owner, desc=desc)
        session.add(data)
        session.commit()
        session.close()
